import type {RiskProfile} from '../lib/analysis/types.ts';
...
export async function initializeDatabase(db:D1Database, profile:RiskProfile){
  await db.batch(schemaStatements.map(statement=>db.prepare(statement)));
  await db.prepare(
    `INSERT INTO profile_versions
      (id, profile_id, version, prompt_version, name, status, definition_json, created_at)
